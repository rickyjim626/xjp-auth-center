...
export async function authRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/auth/wechat/qr', async (request, reply) => {
    try {
      ...
      const result = await wechatService.generateQRCode(
        body.client_id,
        body.redirect_uri
      );
  ...
  fastify.get('/auth/wechat/callback', async (request, reply) => {
    try {
      ...
      const result = await wechatService.handleCallback(query.code, query.state);
  ...
  fastify.get('/auth/login-stream', async (request, reply) => {
    try {
      ...
      const status = await wechatService.getLoginTicketStatus(query.loginId);
  ...
  fastify.get('/auth/login-status/:loginId', async (request, reply) => {
    ...
    try {
      const status = await wechatService.getLoginTicketStatus(loginId);
